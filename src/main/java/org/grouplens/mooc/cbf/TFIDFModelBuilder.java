...
public class TFIDFModelBuilder implements Provider<TFIDFModel> {
    ...
    public TFIDFModel get() {
        ...
        for (long item: items) {
            ...
            List<String> tags = dao.getItemTags(item); //vector of all tags of an item
            ...
            for (VectorEntry e: work.fast(VectorEntry.State.SET)) {
            	Long tagId  = e.getKey();
            	docFreq.add(tagId, 1.0); 
            }
            //System.out.println(""+docFreq.get(tagIds.get("capitalism")));
        ...
        for (VectorEntry e: docFreq.fast()) {
        	...
        	//System.out.println(""+(double)dao.getItemIds().size()+" / "+df);
        	Double logIDF = Math.log10( (double)dao.getItemIds().size() / df);
        ...
        for (Map.Entry<Long,MutableSparseVector> entry: itemVectors.entrySet()) {
            ...
            for (VectorEntry tagEntry: tv.fast()) {
            	Long tagId = tagEntry.getKey();
            	Double TF = tagEntry.getValue();
            	Double TFIDF = TF * docFreq.get(tagId);
            	tv.set(tagEntry, TFIDF);
            }
            ...
            Double norm = tv.norm();
            for (VectorEntry tagEntry: tv.fast()) {
            	Double notNormalized = tagEntry.getValue();
            	Double normalized = notNormalized / norm;
            	tv.set(tagEntry, normalized);
            }
        ...
        /*SparseVector tv = modelData.get(11L);
        for (VectorEntry tagEntry: tv.fast()) {
        	//tagEntry.getKey();
        	//tagEntry.getValue();
        	System.out.println(""+tagEntry.getKey()+":"+tagEntry.getValue());
        }
        */
