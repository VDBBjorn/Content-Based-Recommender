...
public class TFIDFItemScorer extends AbstractItemScorer {
	...
	public void score(long user, @Nonnull MutableSparseVector output) {
		...
		for (VectorEntry e: output.fast(VectorEntry.State.EITHER)) {
			...
			Double squaredProfileVector = 0.0;
			Double squaredItemVector = 0.0;
			for (VectorEntry tagEntry: iv.fast()) {
				Long tagId = tagEntry.getKey();
				Double itemValue = tagEntry.getValue();
				Double profileValue = userVector.get(tagId);
				numerator += itemValue*profileValue;	
				squaredItemVector += itemValue*itemValue;
			}
			for(VectorEntry tagEntry: userVector.fast()){
				Double profileValue = tagEntry.getValue();
				squaredProfileVector += profileValue*profileValue;
			}
			denominator = Math.sqrt(squaredProfileVector)*Math.sqrt(squaredItemVector);
			//denominator = iv.norm()*userVector.norm();
			Double cosineSim = numerator / denominator; 
			output.set(e,cosineSim);
	...
	private SparseVector makeUserVector(long user) {
		...
		for (Rating r: userRatings) {
			...
			if(!weighted){ //unweighted option for profile building
				if (p != null && p.getValue() >= 3.5) {
					...
					SparseVector TFIDFVector = model.getItemVector(itemId);
					for (VectorEntry tagEntry: TFIDFVector.fast()) {
						Long tagId = tagEntry.getKey();
						Double TFIDF = tagEntry.getValue();

						profile.add(tagId, TFIDF);
			...
			else{//weighted option for profile building
				...
				if (p != null) {	
					...
					for (VectorEntry tagEntry: TFIDFVector.fast()) {
						...
						double weight = p.getValue() - getAverageUserRating(user);
						profile.add(tagId, TFIDF*weight);
					}
				}
			}
	...
	private double getAverageUserRating(long user){
		double sum = 0.0;
		int numberRatings = 0;
		...
		if(numberRatings>0){
			return sum / (double)numberRatings;
		}
		else{
			return 0.0;
		}
